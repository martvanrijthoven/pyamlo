...
class SecurityPolicy:
    ...
    def __init__(
        ...
        self.allowed_env_vars = (
            set() if allowed_env_vars is None else set(allowed_env_vars)
        )
        self.allowed_imports = (
            set() if allowed_imports is None else set(allowed_imports)
        )
        self.allowed_include_paths = (
            set() if allowed_include_paths is None else set(allowed_include_paths)
        )

        if restrictive:
            self.allow_expressions = (
                False if allow_expressions is None else allow_expressions
            )
        else:
            self.allow_expressions = (
                True if allow_expressions is None else allow_expressions
            )
